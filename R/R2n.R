...
#' Function that computes Rn_iid_integrated(eps) (denoted with a bar)
#' Defined in C.3.2
#'
#' @examples
...
Rn_iid_integrated <- function(eps, noskewness, n, K4, lambda3 = NULL)
{
  ...
  tildeA6n <- bound_modulus_psi / pi * val_e_2n / (8 * n^2) *
    (K4 / 12 +  1 / (4 * (1 - 3*eps)^2) + val_P_3n / (576 * (1- 3*eps)^2)) *
    2^(5) * Standard_gamma(6)
...
#' Function that computes P_{3,n}(eps)
#' Defined in C.3.2
#'
#' @examples
#' P_3n(eps = 0.1, noskewness = TRUE, n = 500, K4 = 9)
#' P_3n(eps = 0.1, noskewness = FALSE, n = 500, K4 = 9, lambda3 = 6)
#'
#' TODO: renommer dans le papier en P_2n, ok
#'
#'
#' @noRd
#'
P_3n <- function(eps, noskewness, n, K4, lambda3 = NULL){
...
e_2n <- function(eps, noskewness, n, K4, lambda3 = NULL){
  ...
  val_e_2n <- exp(eps^2 *
                    (1/6 + 1/(2 * (1 - 3*eps)^2) +
                       2 * val_P_3n / (576 * (1 - 3*eps)^2)))

  return(val_e_2n)
}

# TODO: a checker avec papier si obsolete ou encore besoin?
# Function P_{2,n}(eps)
# P_2n <- function(eps, lambda3, K4, n){
#
#   val_P_3n <- P_3n(eps = eps, lambda3 = lambda3, K4 = K4, n = n)
#
#   val_P_2n <- 1/4 + val_P_3n/576
#
#   return(val_P_2n)
# }
